...
| [Fork PR #24](https://github.com/kaizhangNV/slang/pull/24)                                                                         | Structural ray-tracing API and frontend contracts     | PR 1; checkpoint `e285e1e2b`                          |
...
PR 1 checkpoint `e285e1e2b` is one commit on PR #13163 checkpoint `e85404e2f`; the stack is
rebased on `master` commit `bc2280c77`.

- Release `slang-test` build passed with four build jobs (309/309 build steps).
- `tests/ray-tracing-2`: 79/79 passed.
- Structural entry-point and module-import unit regressions: 9/9 passed.
...
- The experimental-module unit-test object has no undefined private `Linkage`, `Module`, or RTTI
  references.
