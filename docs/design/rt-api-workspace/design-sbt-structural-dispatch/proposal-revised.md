...
| 2.2.3, payload model | Every `trace` passes a payload value | `trace(desc, as, descriptor)`, without a payload argument, for entries whose payload type is **empty** (a struct with no data members); the module defines no such type, a schema may contain at most one, and its value cannot cross the structural API boundary (not as a `trace` argument and not as a stage's `input.payload`) | A value that exists only to be ignored is noise at the most common call site; the type still partitions the schema and selects the Metal tables |
...
namespace rt
{
    ...
    // and no value of an empty payload type is passed through the structural API. The same empty
    // struct remains an ordinary constructible Slang type outside trace and stage-input use.
...
| `StructuralRayTracingEmptyPayloadValue` | an empty payload type crosses a structural API boundary: as a `trace` argument, as an explicit generic argument to `trace`, or as `input.payload` in a stage whose context names it |
